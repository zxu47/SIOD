...
class RandomCrop:
    ...
    def expand(self,image,label):
     ...
     #   print("expand",p)
        if p>0.5:
            return image,label,0,image.shape[1],0,image.shape[0]
        config = self.config
        min_scale = 1.0
        max_scale = 2.0
    ...
    def crop(self,image,label,w_start,w_end,h_start,h_end):
       ...
       # print("crop",p)
        ...
        for i in range(trail):
            h_middle = np.random.randint(h_start+1,h_end-1)
            w_middle = np.random.randint(w_start+1,w_end-1)
          #  print(i,h_middle,w_middle)
            ...
            if ratio<min_scale or ratio>1/min_scale:
                continue
